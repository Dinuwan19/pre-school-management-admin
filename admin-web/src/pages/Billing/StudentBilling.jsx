...
import { Table, Button, Space, Tag, Modal, Form, Input, Select, message, Typography, Card, Badge, Descriptions, Divider, List } from 'antd';
...
const StudentBilling = () => {
    ...
    const handleGenerate = async () => {
        try {
            ...
            setLoading(true);
            await api.post('/billing/generate', values);
                ...
                <Space>
                    ...
                    {record.status === 'PAID' && (
                        <Button
                            icon={<EyeOutlined />}
                            size="small"
                            onClick={() => window.print()} // Simple print for now as "Invoice"
                        >
                            Invoice
                        </Button>
                    )}
        ...
        <div style={{ paddingBottom: 40 }}>
            ...
            >
                <Form form={form} layout="vertical" style={{ marginTop: 20 }}>
                    ...
                    <Form.Item name="billingMonth" label="Billing Month (YYYY-MM)" rules={[{ required: true }]}>
                        <Input placeholder="e.g. 2025-01" />
                    </Form.Item>
                    ...
                    renderItem={(item) => (
                        <Card style={{ marginBottom: 16, borderRadius: 12 }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                                <Descriptions column={2} size="small" title={`Payment from ${item.billingPayments?.[0]?.billing?.student?.fullName}`}>
